...
import {
  LayoutDashboard, Package, Users, MapPin, Truck, Building2,
  Settings, LogOut, Bell, Ticket, Wallet, Activity, UserCheck,
  BarChart2, UserCircle, MessageSquare, CreditCard, Copy,
...
export function Layout({ children }: LayoutProps) {
  ...
  const SidebarContent = () => (
    <>
          ...
          return (
            <Link key={item.href} href={item.href}>
              <Button
                variant="ghost"
                className={cn(
                  "w-full justify-start text-primary-foreground hover:bg-primary-foreground/10 hover:text-primary-foreground h-11",
      ...
      <div className="p-4 border-t border-primary-foreground/20">
        ...
        <Button
          variant="ghost"
          className="w-full justify-start text-primary-foreground hover:bg-red-700 hover:text-primary-foreground h-11"
          onClick={() => logout()}
        >
  ...
  return (
    <div className="flex h-screen bg-background">
      ...
      <div
        className={cn(
          "fixed inset-y-0 left-0 z-50 w-72 flex flex-col bg-primary shadow-xl md:hidden transition-transform duration-300 ease-in-out",
      ...
      <div className="flex flex-1 flex-col md:pl-64 min-w-0">
        {/* Top Header */}
        <header className="flex h-16 items-center justify-between border-b bg-card px-4 sm:px-6 shadow-sm z-10 shrink-0">
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-base sm:text-lg font-medium truncate">{currentPageName}</h1>
          </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="relative h-9 w-9 rounded-full bg-primary text-primary-foreground hover:bg-primary/90 text-sm font-semibold shrink-0"
                >
                  {initials}
                </Button>
              </DropdownMenuTrigger>
        ...
        <main className="flex-1 overflow-auto p-3 sm:p-6 relative min-w-0">
